...
fig1 = go.Figure()
fig1.add_trace(
    go.Scatter(y=hist_vol_roll.tolist(), name="Historical", line=dict(width=1.5))
)
fig1.add_trace(
    go.Scatter(
        y=synth_vol_roll.tolist(),
        name="Synthetic (GARCH)",
        line=dict(width=1.5, dash="dash"),
    )
)
fig1.update_layout(
    title={
        "text": "Rolling 21-Day Volatility — GARCH(1,1)<br>"
...
fig2.update_layout(
    barmode="group",
    title={
        "text": "ACF of Squared Returns — GARCH(1,1)<br>"
        "<span style='font-size:16px;font-weight:normal'>"
...
x_range = np.linspace(
    min(hist.min(), synth.min()) * 1.1, max(hist.max(), synth.max()) * 1.1, 400
)
...
fig3.update_layout(
    title={
        "text": "Return Distribution — GARCH(1,1)<br>"
        "<span style='font-size:16px;font-weight:normal'>"
...
# ── Chart 4: QQ Plot ──────────────────────────────────────────────────
qq_min, qq_max = float(min(hist_q.min(), synth_q.min())), float(
    max(hist_q.max(), synth_q.max())
)
fig4 = go.Figure()
fig4.add_trace(
    go.Scatter(
...
fig4.update_layout(
    title={
        "text": "QQ Plot: Synthetic vs Historical — GARCH(1,1)<br>"
        "<span style='font-size:16px;font-weight:normal'>"
...
fig5.update_layout(
    barmode="group",
    title={
        "text": "ACF of Returns — GARCH(1,1)<br>"
        "<span style='font-size:16px;font-weight:normal'>"
...
colors = [
    "#2ecc71" if s >= 0.8 else "#e67e22" if s >= 0.5 else "#e74c3c" for s in scores
]
...
fig6.update_layout(
    title={
        "text": "GARCH(1,1) Validation Score Summary<br>"
        "<span style='font-size:16px;font-weight:normal'>"

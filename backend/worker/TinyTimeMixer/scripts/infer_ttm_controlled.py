    ...
    *,
    ...
    while remaining > 0:
        ...
        if freq_token_value is not None:
            freq_token = torch.full((x.shape[0],), int(freq_token_value), device=device, dtype=torch.long)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lookback-days", type=int, default=365, help="Lookback days if start not set.")
    parser.add_argument("--horizon", type=int, default=60, help="Forecast horizon (days).")
    ...
    parser.add_argument("--weights", type=str, default=None, help="Path to finetuned weights.")
    parser.add_argument("--scaler", type=str, default=None, help="Path to scaler.")
    parser.add_argument("--config", type=str, default=None, help="Path to config JSON.")
    parser.add_argument("--output-csv", type=str, required=True, help="CSV output path.")
...
def main() -> None:
    ...
    weights_path = Path(args.weights) if args.weights else (default_dir / "ttm_controlled_weights.pt")
    scaler_path = Path(args.scaler) if args.scaler else (default_dir / "ttm_controlled_scaler.pt")
    config_path = Path(args.config) if args.config else (default_dir / "ttm_controlled_config.json")
    ...
    freq_token_value = DEFAULT_FREQUENCY_MAPPING.get(freq_str, None) if use_freq_token else None

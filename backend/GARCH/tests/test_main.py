...
def test_generate_api_success(mock_job_store):
    ...
    assert "Poll /api/status/" in data["message"]
...
def test_status_api_existing_completed_job(mock_job_store):
    mock_job_store.get_status.return_value = "completed"
    mock_job_store.get_parameters.return_value = {
        "omega": 0.1,
        "alpha": 0.2,
        "beta": 0.7,
    }
    mock_job_store.get_metrics.return_value = {"ks_statistic": 0.05}
...
def test_status_api_failed_job(mock_job_store):
    mock_job_store.get_status.return_value = "failed"
    mock_job_store.get_error.return_value = "Model fitting failed"

    response = client.get("/api/status/test-job-failed")
...
def test_status_api_nonexistent_job(mock_job_store):
    mock_job_store.get_status.return_value = None
